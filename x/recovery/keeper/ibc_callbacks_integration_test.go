...
var _ = Describe("Recovery: Performing an IBC Transfer", Ordered, func() {
	...
	Describe("from a non-authorized chain", func() {
		BeforeEach(func() {
			...
			err := s.EvmosChain.App.(*app.Bridge).ClaimsKeeper.SetParams(s.EvmosChain.GetContext(), params)
		...
		It("should transfer and not recover tokens", func() {
			...
			nativeEvmos := s.EvmosChain.App.(*app.Bridge).BankKeeper.GetBalance(s.EvmosChain.GetContext(), senderAcc, "aevmos")
			Expect(nativeEvmos).To(Equal(coinEvmos))
			ibcOsmo := s.EvmosChain.App.(*app.Bridge).BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
		...
		Describe("to the sender's own eth_secp256k1 account on Evmos (sender == recipient)", func() {
			...
			Context("with disabled recovery parameter", func() {
				BeforeEach(func() {
					...
					s.EvmosChain.App.(*app.Bridge).RecoveryKeeper.SetParams(s.EvmosChain.GetContext(), params) //nolint:errcheck
				})
			...
			Context("with a sender's claims record", func() {
				Context("without completed actions", func() {
					BeforeEach(func() {
						...
						s.EvmosChain.App.(*app.Bridge).ClaimsKeeper.SetClaimsRecord(s.EvmosChain.GetContext(), senderAcc, claim)

						// update the escrowed account balance to maintain the invariant
						err := testutil.FundModuleAccount(s.EvmosChain.GetContext(), s.EvmosChain.App.(*app.Bridge).BankKeeper, claimstypes.ModuleName, coins)
						s.Require().NoError(err)
					})
				...
				Context("with completed actions", func() {
					...
					It("should transfer tokens to the recipient and perform recovery", func() {
						...
						balanceEscrow := s.EvmosChain.App.(*app.Bridge).BankKeeper.GetBalance(s.EvmosChain.GetContext(), transfertypes.GetEscrowAddress("transfer", "channel-0"), "aevmos")
					...
					It("should not claim/migrate/merge claims records", func() {
						...
						claimAfter, _ := s.EvmosChain.App.(*app.Bridge).ClaimsKeeper.GetClaimsRecord(s.EvmosChain.GetContext(), senderAcc)
			...
			Context("without a sender's claims record", func() {
				...
				When("recipient has additional ibc vouchers that originated from other chains", func() {
					...
					It("should not recover tokens that originated from other chains", func() {
						...
						ibcAtom := s.EvmosChain.App.(*app.Bridge).BankKeeper.GetBalance(s.EvmosChain.GetContext(), senderAcc, teststypes.UatomIbcdenom)
				...
				When("recipient has additional non-native ibc vouchers that originated from senders chains", func() {
					BeforeEach(func() {
						params := types.DefaultParams()
						params.EnableRecovery = false
						err := s.EvmosChain.App.(*app.Bridge).RecoveryKeeper.SetParams(s.EvmosChain.GetContext(), params)
						s.Require().NoError(err)
						...
						osmoIBCAtom := s.EvmosChain.App.(*app.Bridge).BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, teststypes.UatomOsmoIbcdenom)
