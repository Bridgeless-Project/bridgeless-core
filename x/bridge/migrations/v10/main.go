...
func MigrateStore(ctx sdk.Context, storeKey storetypes.StoreKey, cdc codec.BinaryCodec) error {
	ctx.Logger().Info(fmt.Sprintf("Performing v12.1.30-rc16 %s module migrations", types.ModuleName))
...
func removeCommission(sdkCtx sdk.Context, storeKey storetypes.StoreKey, epochId uint32, tokenId uint64) {
	cStore := prefix.NewStore(sdkCtx.KVStore(storeKey), types.Prefix(types.StoreCommissionPrefix))
	eStore := prefix.NewStore(cStore, types.KeyEpoch(epochId))
