...
build-docker:
	...
	$(DOCKER) rm bridgeless-core || true
	# create a new container from the latest image
	$(DOCKER) create --name bridgeless-core -t -i ${DOCKER_IMAGE}:latest bridgeless-core
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp bridgeless-core:/usr/bin/bridgeless-cored ./build/
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\bridgeless-cored:/bridgeless-core\Z bridgeless-core/node "./bridgeless-cored tendermint unsafe-reset-all --home=/bridgeless-core"
	@docker run --rm -v $(CURDIR)\build\node1\bridgeless-cored:/bridgeless-core\Z bridgeless-core/node "./bridgeless-cored tendermint unsafe-reset-all --home=/bridgeless-core"
	@docker run --rm -v $(CURDIR)\build\node2\bridgeless-cored:/bridgeless-core\Z bridgeless-core/node "./bridgeless-cored tendermint unsafe-reset-all --home=/bridgeless-core"
	@docker run --rm -v $(CURDIR)\build\node3\bridgeless-cored:/bridgeless-core\Z bridgeless-core/node "./bridgeless-cored tendermint unsafe-reset-all --home=/bridgeless-core"
else
	@docker run --rm -v $(CURDIR)/build/node0/bridgeless-cored:/bridgeless-core:Z bridgeless-core/node "./bridgeless-cored tendermint unsafe-reset-all --home=/bridgeless-core"
	@docker run --rm -v $(CURDIR)/build/node1/bridgeless-cored:/bridgeless-core:Z bridgeless-core/node "./bridgeless-cored tendermint unsafe-reset-all --home=/bridgeless-core"
	@docker run --rm -v $(CURDIR)/build/node2/bridgeless-cored:/bridgeless-core:Z bridgeless-core/node "./bridgeless-cored tendermint unsafe-reset-all --home=/bridgeless-core"
	@docker run --rm -v $(CURDIR)/build/node3/bridgeless-cored:/bridgeless-core:Z bridgeless-core/node "./bridgeless-cored tendermint unsafe-reset-all --home=/bridgeless-core"
endif
