...
import (
	...
	"github.com/Bridgeless-Project/bridgeless-core/v12/tests/e2e/upgrade"
...
func (s *IntegrationTestSuite) upgrade(targetRepo, targetVersion string) {
	...
	// export node .bridgeless-cored to local build/
	...
	node.SetCmd([]string{"bridgeless-cored", "start", fmt.Sprintf("--chain-id=%s", s.upgradeParams.ChainID)})
	err = s.upgradeManager.RunNode(node)
