...
import (
	...
	"github.com/Bridgeless-Project/bridgeless-core/v12/testutil/tx"
...
func DeployContract(
	ctx sdk.Context,
	bridgeApp *app.Bridge,
	priv cryptotypes.PrivKey,
	queryClientEvm evm.QueryClient,
	contract evm.CompiledContract,
	constructorArgs ...interface{},
) (common.Address, error) {
	chainID := bridgeApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	nonce := bridgeApp.EvmKeeper.GetNonce(ctx, from)
	...
	msgEthereumTx := evm.NewTx(&evm.EvmTxArgs{
		...
		Nonce:     nonce,
		GasLimit:  gas,
		GasFeeCap: bridgeApp.FeeMarketKeeper.GetBaseFee(ctx),
	...
	res, err := DeliverEthTx(bridgeApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := CheckEthTxResponse(res, bridgeApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func DeployContractWithFactory(
	...
	chainID := bridgeApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	factoryNonce := bridgeApp.EvmKeeper.GetNonce(ctx, factoryAddress)
	nonce := bridgeApp.EvmKeeper.GetNonce(ctx, from)
