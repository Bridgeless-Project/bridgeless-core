...
var (
	...
	DefaultFee = sdk.NewCoin(utils.BaseDenom, sdk.NewIntFromUint64(uint64(feeAmt))) // 0.01 Bridge
)
...
type CosmosTxArgs struct {
	...
	// ChainID is the chain's id on cosmos format, e.g. 'bridge_9000-1'
	ChainID string
...
func signCosmosTx(
	...
	seq, err := appBridge.AccountKeeper.GetSequence(ctx, addr)
	if err != nil {
		return nil, err
	}
	...
	accNumber := appBridge.AccountKeeper.GetAccount(ctx, addr).GetAccountNumber()
