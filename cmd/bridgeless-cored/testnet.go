...
Example:
	bridgeless-cored testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "bridgeless-cored", "Home directory of the node's daemon configuration")
...
Example:
	bridgeless-cored testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	if args.chainID == "" {
		args.chainID = fmt.Sprintf("bridgeless-core_%d-1", tmrand.Int63n(9999999999999)+1)
	}
